...
public class UpdateGamedata extends BaseAction{
	...
	public String UpdateRank(){
		...
		if(mode.equals("rank")){                //排位模式
			int rankpoint = RankPoint(num-wrong,wrong,result);
	...
	public int RankPoint(int correct,int wrong,String result){
		GameData gamedata = (GameData)session().getAttribute("gamedata");
		double rate = gamedata.getRankwin()/gamedata.getRankgame();
		double rankpoint = 0;
		
		if(result.equals("win")){
			rankpoint = 100*(1+rate)+correct*10;
		}
		else{
			
			rankpoint = 100*(1-rate)-correct;
			if(rankpoint<0)
				rankpoint = 0;
			rankpoint *=-1;
		}
		return (int)rankpoint;
	}
